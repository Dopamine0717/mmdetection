...
def showBBox(coco, anns, label_box=True, is_filling=True):
    ...
    for cat in coco.getCatIds():
        image2color[cat] = (np.random.random((1, 3)) * 0.7 + 0.3).tolist()[0]    # 这是干嘛？
    for ann in anns:
...
def show_coco(data_root, ann_file, img_prefix, only_bbox=True, show_all=True, category_name='bicycle'):
    example_coco = COCO(ann_file)    # 用于读取和可视化注释的COCO辅助类。
    ...
    for i in range(len(image_ids)):
        plt.figure()
        image_data = example_coco.loadImgs(image_ids[i])[0]    # 取零是因为会有重复的出现，比如多个物体对应一张图片
