...
def show_loss_grad(aux_predict_out, out_losss, out_grads, label, legend_list, extra_str=''):
    ...
    # 自己加的代码
    aux_predict_out2 = aux_predict_out.copy()
    for i, ts in enumerate(aux_predict_out2):
        # ts.requires_grad_()
        print(aux_predict_out2[i])
        # ts.detach().numpy()
        aux_predict_out2[i].detach().cpu().numpy()
        print(aux_predict_out2[i])

    for loss in out_losss:
        p = plt.plot(aux_predict_out2, loss)   # TODO:
        pp.append(p)
    ...
    for grad in out_grads:
        p, = plt.plot(aux_predict_out2, grad)   # TODO:
        pp.append(p)
    plt.legend(pp, list(map(str, legend_list)))
    plt.show()
